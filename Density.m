...
axis tight;
        ...
        h=figure('Visible',visible); 
h2=subplot('position',subplotstack(5,2));plot((1:length(AVMDs))-25,AVs(1,:,6),'+-'); ylabel('V_{SW} (km/s)');%V_sw
h1=subplot('position',subplotstack(5,1));plot((1:length(AVMDs))-25,AVs(1,:,5),'+-'); ylabel('Bz (nT)'); %Bz
h3=subplot('position',subplotstack(5,3));plot((1:length(AVMDs))-25,AVs(1,:,15),'+-');ylabel('D_{ST} (nT)');%dst
h4=subplot('position',subplotstack(5,4));plot((1:length(AVMDs))-25,AVs(1,:,29),'+-');ylabel('F10.7 (s.f.u.)');%f107
set(findobj('type','axes'),'xticklabel',{[]})
        stime=(1:length(AVMDs))-25;
