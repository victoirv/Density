...
A similar hypothesis test run for the bottom panel of Figure~\ref{DailyAveragedDstEvents} yields a maximum confidence level of $\sim$85\%.
...
We have also considered the hourly averages from the events shown in the bottom panel of Figure~\ref{DailyAveragedDstEvents}, which shown in the top panel of Figure~\ref{HourlyAveragedDstEvents}.  In the time period of elevated mass density within 6 hours of the start of the event, there are only $\sim$4 samples per hourly interval and the peak value of $\sim$70 amu/cm$^3$ is associated with a single observation.  

To increase the number of events, we have removed the constraint that the events must occur in the MLT range from 06:00-12:00 and the constraint that $D_{st}$ remained below the threshold value for 12 hours or longer.  The result is shown in the bottom panel of Figure~\ref{HourlyAveragedDstEvents}. The two spikes (one upwards after onset, one downwards before onset) were tested for significant deviation from two baselines, one 24-12 hours before onset, one 16-48 hours after onset. The probability of the left spike coming from the same distribution as the left baseline is 1.06\%, and 8.2\% from the right baseline, whereas the probability of the right spike coming from the left baseline is 1.89\% and 0.17\% from the right baseline.  This indicates that there is a statistically significant peak in $\rho_{eq}$ within 6 hours of onset when hourly averages are considered.  This result is consistent with previous observations of large mass density increases in the plasma trough region during large geomagnetic storms (\cite{Yao2008}; \cite{Takahashi2010}).  
...
Figure~\ref{BzBinned} shows the the events of Figure~\ref{HourlyAveragedRhoEvents} when separated by $B_z$ in the same manner as Figure~\ref{fig:BzBinnedDstEvents}.  The largest difference occurs when the separation is performed after onset, with more positive $B_z$ leading to a larger peak in $\rho_{eq}$ than more negative $B_{z}$.  The rate of change of $\rho_{eq}$ also appears to be insenstive to $B_z$ for approximately 12 hours prior to onset.  
