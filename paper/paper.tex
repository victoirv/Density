...
We have also considered the hourly averages from the events shown in the bottom panel of Figure~\ref{HourlyAveragedDstEvents}, which shown in the top panel of Figure~\ref{HourlyAveragedDstEvents}.  In the time period of elevated mass density within 6 hours of the start of the event, there are only $\sim$4 samples per hourly interval.  The peak value of $\sim$70 amu/cm$^3$ is associated with a single observation.  

To increase the number events, we have remove the constraint that the events must occur in the MLT range from 06-12 and the constraint that $D_{st}$ remained below the threshold value for 12 hours or longer.  The result is shown in the bottom panel of Figure~\ref{HourlyAveragedDstEvents}.
