%%  SAMPLE.TEX - A LaTeX document template for GRL manuscripts
%%  Jan 21, 1997 version
%%  
   % This document demonstrates how to use commands 
   % described in the aguguide.tex user manual to create GRL
   % manuscripts for on-line submission.  Most 
   ...
   % You may use this file as a LaTeX document template for 
   % your GRL manuscript by replacing the sample text with 
   % your own text and deleting any extraneous commands.


%%   PLEASE NOTE:
%%   1) Don't include any extra style files.
%%   2) Use LaTeX "thebibliography" environment for the reference
%%      list.  Use \cite to cite references in the main text.
%%
%% ------------------------------------------------------ %%

   % The first item in a LaTeX file should be a \documentstyle
   % command to declare the overall style of the paper.  The
   % \documentstyle line that is relevant for GRL manuscripts
   % is shown below. 

\documentstyle[12pt,agums]{article}
    % The agums (manuscript) style creates double-spaced wide-margined 
    % manuscripts.  Use it to create copy to submit to 
    % GRL editors for first-time review.

%\documentstyle[grlga]{article}
    % The grlga (galley) style can be used to create 
    % camera-ready pages. You can use this to measure 
    % the size of your manuscript.
...
\righthead{LATEX SAMPLE DOCUMENT FOR GRL MANUSCRIPTS}
...
\ccc{0148-0227/97/97GL-00000\$05.00}
...
ls@red.whoi.edu; sw@red.whoi.edu)}
   ...
   % includes an underscore, do not forget
   % to place a backslash in front of it or it will 
...
\slugcomment{To appear in the {\it Geophysical Research  
Letters}, 1996.}
...
%% ------------------------------------------------------ %%
  ...
  % \input{update.tex}
   ...
   % ONLY if you are using a VERY OLD version of LaTeX 2.09 
   % and include \cite commands, LaTeX may have trouble 
   % with the \reset@font command.  If so, LaTeX will pause 
   % at every undefined citation the first time the input 
   % file is LaTeXed.  Authors may either press [Enter] 
   % each time this occurs in their first LaTeX run, or 
   % they may type \input{update.tex} in front 
   % of the \begin{document} command.  The "update.tex" file 
   % is available on the kosmos.agu.org anonymous ftp site.
...
%%
   ...
   % an \end{article} command after the bibliography 
...
%% ------------------------------------------------------ %%
   ...
   % Use "\cite" commands to call out citations,
   ...
   % command, for example, \cite{can95}
   %
   % Try to use \cite for all references in your document. 
   % Use \markcite commands for the format such as 
   % {\it Kurtz and King} [1994].
   % HOWEVER, THIS FORMAT SHOULD BE USED AS LESS AS POSSIBLE.
   %


\begin{thebibliography}{}
\bibitem[{\it Baker et al.,} 1977]{baker77}
...
\bibitem[{\it Degges and Smith,} 1977]{degges77}
...
\bibitem[{\it Golitsyn and Mokhov,} 1995]{golitsyn95}
...
\bibitem[{\it McMullen,} 1978]{mcmullen78}
\reference % (Thesis)
...
\bibitem[{\it Namias,} 1974]{namias74}
...
\bibitem[{\it Yamazaki,} 1978]{yamazaki78}
