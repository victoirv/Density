...
\cite{Gallagher1988EmpiricalModelPlasmasphere} \vinote{Too far back?} first models the the plasmasphere as a simple linear fit dependent on L shell, geographic height, geomagnetic latitude, and $K_p$. \cite{Hamilton1988RingCurrentDevelopment} simultaneously analyzes a strong geomagnetic storm by comparing empirical energy measurements of the ring current to $D_{st}$, along with measuring ring current compositions before, during, and after the storm. 

\cite{Lotoaniu1999PlasmaMassDensity} uses CRRES to compare in-situ measured plasma mass density to that inferred by ground-based ULF wave measurements extended to the plasmatrough. 
