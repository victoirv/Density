...
In this work events are considered when $D_{st}$ or $\rho_{eq}$ crosses a threshold value, as indicated in \ref{AllData}.  In finding events, all fill values were replaced with linearly interpolated values.  Figure \ref{AllData} shows the values of $B_Z$, $V_{SW}$, $D_{st}$, $F_{10.7}$, and Mass Density for the duration of the dataset. 
...
\caption{Overview of data used in this study. The top four panels are from \cite{Reconstruction} and the bottom panel is based on \cite{Denton} after interpolation and averaging described in the text. Dashed horizontal lines indicate default event cutoff thresholds.}
\label{AllData}
\end{figure}
...
\caption{(a) All data of $D_{st}$ and $\rho_{eq}$ (b) Correlation between $log(\rho_{eq\_27d})$ and $F_{10.7\_27d}$. (c) $D_{st}$ against $\rho_{eq}$ at one hour and one-day median values.  }
\label{ccplot}
\end{figure}
\clearpage
